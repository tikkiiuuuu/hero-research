...
namespace io
{
HikRobot::HikRobot(double exposure_ms, double gain, const std::string & vid_pid, const int option)
: exposure_us_(exposure_ms * 1e3), gain_(gain), queue_(1), queue_both_(1), daemon_quit_(false), vid_(-1), pid_(-1)
{
  set_vid_pid(vid_pid);
  if (libusb_init(NULL)) tools::logger()->warn("Unable to init libusb!");

  daemon_thread_ = std::thread{[this, option ] {
    tools::logger()->info("HikRobot's daemon thread started.");

    capture_start(option);
...
void HikRobot::read_bayerboth(cv::Mat & img_bgr, cv::Mat & img_bayer, std::chrono::steady_clock::time_point & timestamp)
{
  CameraBayerBothData data;
  queue_both_.pop(data);

  img_bgr = data.img_bgr;
  img_bayer = data.img_bayer;
  timestamp = data.timestamp;
}

void HikRobot::capture_loop()
{
...
void HikRobot::capture_loop_bayerboth()
{
  tools::logger()->info("HikRobot's capture thread started bayer.");
  ...
  while (!capture_quit_) {
    ...
    queue_both_.push({img,img_bayer, timestamp});
...
void HikRobot::capture_start(const int option)
{
  capturing_ = false;
  capture_quit_ = false;
  ...
  set_enum_value("PixelFormat", PixelType_Gvsp_BayerRG8);//* 新增
  set_float_value("ExposureTime", exposure_us_);
  set_float_value("Gain", gain_);
  MV_CC_SetFrameRate(handle_, 150);

  // if(option == 1)ret = MV_CC_SetBayerCvtQuality(handle_, 0);
  // else ret = MV_CC_SetBayerCvtQuality(handle_, 0);
  //* 不采用海康的插值
  ...
  //TODO 暂时这样吧，懒得写了
  if(option == 0)capture_thread_ = std::thread(& HikRobot::capture_loop,this);
  else capture_thread_ = std::thread(& HikRobot::capture_loop_bayerboth,this);
}
