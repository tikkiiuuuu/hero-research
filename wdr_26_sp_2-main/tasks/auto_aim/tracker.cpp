  ...
  omni_target_priority_{ArmorPriority::fifth}
{
  ...
  filter = yaml["filter_type"].as<std::string>();
  enemy_color_ = (yaml["enemy_color"].as<std::string>() == "red") ? Color::red : Color::blue;
  min_detect_count_ = yaml["min_detect_count"].as<int>();
  max_temp_lost_count_ = yaml["max_temp_lost_count"].as<int>();
  outpost_max_temp_lost_count_ = yaml["outpost_max_temp_lost_count"].as<int>();
  normal_temp_lost_count_ = max_temp_lost_count_;
  R_dig = {yaml["yaw_variance"].as<float>(),
            yaml["pitch_variance"].as<float>(),
            yaml["distance_variance"].as<float>(),
            yaml["orientation_variance"].as<float>()};
}
  ...
  std::list<Armor> & armors, std::chrono::steady_clock::time_point t, bool use_enemy_color)
{
  ...
  // 收敛效果检测：
  if (state_ != "lost" && target_.has_bad_convergence()) {
...
bool Tracker::set_target(std::list<Armor> & armors, std::chrono::steady_clock::time_point t)
{
  ...
  if(filter == "ekf"){     
    ...
    if (is_balance) {
      radius = 0.2;
      armor_num = 2;
      P0_dig.resize(11); P0_dig << 1, 64, 1, 64, 1, 64, 0.4, 100, 1, 1, 1;
    } else if (armor.name == ArmorName::outpost) {
      radius = 0.2765;
      armor_num = 3;
      P0_dig.resize(11); P0_dig << 1, 64, 1, 64, 1, 81, 0.4, 100, 1e-4, 0, 0;
    } else {
      radius = 0.2;
      armor_num = 4;
      P0_dig.resize(11); P0_dig << 1, 64, 1, 64, 1, 64, 0.4, 100, 1, 1, 1;
    ...
    x0 << center_x, 0, center_y, 0, center_z, 0, ypr[0], 0, radius, 0, 0;
    Eigen::MatrixXd P0 = P0_dig.asDiagonal();
    auto x_add = [](const Eigen::VectorXd & a, const Eigen::VectorXd & b) -> Eigen::VectorXd {
      Eigen::VectorXd c = a + b;
      c[6] = tools::limit_rad(c[6]); // 索引6是角度'a'
      return c;
    };
    auto filter_strategy = std::make_unique<EKFStrategy>(t,x0, P0, x_add);
    target_ = Target(armor, armor_num, std::move(filter_strategy));
    //* 只有这里有target的初始化
    target_.R_dig = this->R_dig;
    state_ = "tracking";
  }
  return true;
}
