  ...
  bool to_now)
{
  ...
  // auto ekf = target->ekf(); 
  double delay_time =
    target->get_state()[Target::state::w] > decision_speed_ ? high_speed_delay_time_ : low_speed_delay_time_;
...
{
  const Eigen::VectorXd& ekf_x = target.get_state();
  ...
  // 不考虑小陀螺
  if (std::abs(ekf_x[Target::state::r]) <= 2 && target.name != ArmorName::outpost) {
    // 选择在可射击范围内的装甲板
