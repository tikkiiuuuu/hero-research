 ...
 * @brief   计算到中心距离函数
 ***********************************************************************************************************************/
 float Solver::calculateDistanceToCenter(const cv::Point2f & image_point) const
 {
...
Eigen::Vector3d Solver::odom2camera_tvec() const
{
  return t_odom2camera_;
}
Eigen::Matrix4d Solver::odom_camera_matrix() const
{
  // Eigen::Matrix4d odom_camera_matrix = Eigen::Matrix4d::Identity();
  // odom_camera_matrix.block<3, 3>(0, 0) = R_odom2camera_;
  // odom_camera_matrix.block<3, 1>(0, 3) = t_odom2camera_;
  // return odom_camera_matrix;
  // 正确的从 odom (world) 到 camera 的变换矩阵
  Eigen::Matrix3d R_world2camera = R_camera2gimbal_.transpose() * R_gimbal2world_.transpose();
  Eigen::Vector3d t_world2camera = R_camera2gimbal_.transpose() * (-t_camera2gimbal_);
  
  Eigen::Matrix4d odom_camera_matrix = Eigen::Matrix4d::Identity();
  odom_camera_matrix.block<3, 3>(0, 0) = R_world2camera;
  odom_camera_matrix.block<3, 1>(0, 3) = t_world2camera;
  return odom_camera_matrix;
}
}  // namespace auto_aim